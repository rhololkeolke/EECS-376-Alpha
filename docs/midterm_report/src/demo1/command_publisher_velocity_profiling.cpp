...
// this class integrates the velocity commands and keeps track of
...
// Pass Publisher by reference so that it doesn't need to be
// reinitialized every function call
void straight(ros::Publisher& pub, double distance)
{

  geometry_msgs::Twist vel_object; // stores the velocity commands

  ros::Rate naptime(RATE); // the rate in HZ that this function should run

  double dt = 1/RATE; // time step for integration
  ...
  while(ros::ok() && segDistDone < segLength) { // while not quit and
                                                //still more to go
    ...
    // you've worked hard, so take a nap
    naptime.sleep();
  }
  // publish 0 to make sure the robot is completely stopped
...
// same thing as straight but with turns
...
{
  ros::init(argc,argv,"command_publisher_velocity_profiling"); // name of this node
  ros::NodeHandle n;
  ...
  // the path segments we wish to follow.
  straight(pub,4.2);
  turn(pub,-PI/2);
  straight(pub,12.5);
  turn(pub,-PI/2);
