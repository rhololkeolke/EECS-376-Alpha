...
int main(int argc, char **argv)
{
  ...
  {
    ros::spinOnce(); // service the callbacks
    if(segComplete == true) // time for a new segment
    {
      seg_number++; // increment the number
      segComplete = false;

      switch(seg_number) // each segment number has an associated segment
      {
      case 1:
    ...
    // publish each loop.
    // originally we only published once, but the nodes seemed to miss
    // messages
    //sometimes so we fixed it by publishing always
